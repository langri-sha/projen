...
export interface UvPackageOptions extends PyProjectOptions {
  /**
   * Whether to write a sample `src/<module>/__init__.py` into a new package,
   * so that it builds before anything has been written into it. A package
   * that already has a `pyproject.toml` is never scaffolded.
   *
   * @default true
   */
...
export class UvPackage extends Component {
  ...
  constructor(project: Project, options: UvPackageOptions = {}) {
    ...
    // Only a new package, as `uv init` refuses to run over an existing
    // manifest: the code of one being adopted lives somewhere already, and
    // `src/` appearing beside a flat layout moves setuptools' discovery onto
    // the sample. Read before synthesis, which rewrites the manifest.
    const scaffold =
      sampleCode && !existsSync(join(project.outdir, 'pyproject.toml'))
